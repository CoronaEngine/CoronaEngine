...
class Actor:
    ...
    def _broadcast_actor_created(self):
        ...
        try:
            network_sync_policy.publish_actor_created(
                ...
                emit=lambda actor_data: emit_compat_editor_event(
    ...
    def _broadcast_actor_transform_updated(self):
        ...
        try:
            ...
            emit_compat_editor_event("actor-transform-sync-broadcast", [payload])
        except Exception as exc:
    ...
    def _local_model_library_resource_subdir(rel_path: str) -> Optional[str]:
        ...
        for prefix in ("Resource/", "assets/"):
            library_prefix = prefix + "local_model_library/"
            if normalized.startswith(library_prefix):
                local_rel = normalized[len(prefix):]
    ...
    def set_camera_lock_enabled(self, enabled: bool):
        if self._camera_locked_script is not None:
            ...
            from script_runtime.engine.entities.camera_locked_object import CameraLockedObject
