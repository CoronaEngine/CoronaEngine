...
export function normalizeStoryMapAabb(value) {
  ...
  return {
    min: [Math.min(values[0], values[3]), Math.min(values[1], values[4]), Math.min(values[2], values[5])],
    max: [Math.max(values[0], values[3]), Math.max(values[1], values[4]), Math.max(values[2], values[5])],
  };
}
...
function markerKind(actor = {}) {
  const semanticRole = String(actor.semantic_role || '').trim().toLowerCase();
  const actorType = String(actor.actor_type || actor.type || '').trim().toLowerCase();
...
export function storyMapMarkerFromActor(actor = {}, index = 0) {
  ...
  return {
    id: String(actor.actor_guid || actor.entity_id || actor.handle || actor.name || `actor-${index}`),
...
export function buildStoryMapSnapshot(snapshot = {}, playerPosition = null) {
  ...
  return {
    ...
    markers,
    bounds: nativeBounds ? addStoryMapPadding(nativeBounds) : storyMapBoundsFromPoints(fallbackPoints),
