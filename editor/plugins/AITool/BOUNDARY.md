...
| `cai_extensions/agent/scene_composer.py` 导入后处理 | SceneComposer Actor 变换和物理后处理编排 | 通过 native-first Actor value object 获取视图；native 写入走聚合契约 | 不直接导入 legacy Scene/manager，不定义第二套场景或物理 schema |
