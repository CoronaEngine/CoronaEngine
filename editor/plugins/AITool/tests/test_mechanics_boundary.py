...
def test_mechanics_diagnostic_prefers_native_snapshot_values():
    ...
    assert "native_actor_views_with_legacy_fallback" in diagnostic
    assert "get_legacy_scene" not in diagnostic


def test_mechanics_settlement_remains_explicitly_legacy_until_actor_contract_exists():
    ...
    assert "set_native_actor_physics" not in settlement
...
def test_wall_object_fix_prefers_native_transform_contract():
    ...
    assert "find_actor_with_legacy_fallback" in wall_fix
